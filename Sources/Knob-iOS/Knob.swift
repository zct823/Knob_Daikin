...
open class Knob: UIControl {
    ...
    private var _trackRadius: CGFloat = 0.0
    public var trackRadius: CGFloat {
        get {
            return expanse / 2 - trackLineWidth
        } set (newValue) {
            self._trackRadius = newValue
        }
    }
    ...
    public var startAngle: CGFloat = -.pi / 180.0 * 225.0 {
        didSet {
            ...
            self.negativePieLayer.setNeedsDisplay()
            self.positivePieLayer.setNeedsDisplay()
        }
    }
    ...
    private let negativePieLayer = CAShapeLayer()
    private let positivePieLayer = CAShapeLayer()
...
extension Knob {
    ...
    private func initialize() {
        ...
        for layer in [trackLayer, ticksLayer, progressLayer] {
            firstLayer = layer
        ...
        for layerTwo in [indicatorLayer] {
            secondLayer = layerTwo
            layerTwo.needsDisplayOnBoundsChange = true
            layerTwo.delegate = layerDelegate
            layerTwo.fillColor = UIColor.clear.cgColor
            layerTwo.backgroundColor = UIColor.clear.cgColor
    ...
    private func createPie(piece: (UIBezierPath, UIColor)) -> CAShapeLayer {
